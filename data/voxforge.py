...
parser = argparse.ArgumentParser(
    description='Processes and downloads VoxForge dataset.')
parser.add_argument("--target-dir", default='voxforge_dataset/',
...
def _get_recordings_dir(sample_dir, recording_name):
    ...
    raise Exception(
        "wav or flac directory was not found for recording name: {}".format(recording_name))
...
def prepare_sample(recording_name, url, target_folder):
    ...
    filename_set = set(['_'.join(wav_file.split('_')[:-1])
    ...
    with tempfile.NamedTemporaryFile(suffix=".tgz", mode='wb') as target_tgz:
        ...
        recordings_type, recordings_dir = _get_recordings_dir(
            dirpath, recording_name)
        tgz_prompt_file = os.path.join(
            dirpath, recording_name, "etc", "PROMPTS")

        if os.path.exists(recordings_dir) and os.path.exists(tgz_prompt_file):
            transcriptions = open(tgz_prompt_file).read().strip().split("\n")
            transcriptions = {t.split()[0]: " ".join(
                t.split()[1:]) for t in transcriptions}
...
if __name__ == '__main__':
    ...
    for f in tqdm(all_files, total=len(all_files)):
        prepare_sample(f.replace(".tgz", ""),
                       VOXFORGE_URL_16kHz + f, target_dir)
    print('Creating manifests...')
    create_manifest(target_dir, 'voxforge_train_manifest.csv',
